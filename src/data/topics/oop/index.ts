    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          "id": "oop-q17",
          "kind": "mcq",
          "prompt": "Coming from Python, you write `class Cat : Animal` but expect Cat to also inherit from `Pet`. What does C# do if you try `class Cat : Animal, Pet` where both are classes?",
          "options": [
            {
              "label": "Compile error — a class can inherit from at most ONE base class (single inheritance)",
              "correct": true
            },
            {
              "label": "It works fine — C# supports multiple class inheritance like Python",
              "correct": false
            },
            {
              "label": "It silently picks `Animal` and ignores `Pet`",
              "correct": false
            },
            {
              "label": "It merges the two base classes into one combined type",
              "correct": false
            }
          ],
          "explanation": "Unlike Python's multiple inheritance, C# allows only a SINGLE base class — `class Cat : Animal, Pet` is a compile error if both are classes. You can, however, inherit one base class and implement many interfaces (`class Cat : Animal, IPettable, IFeedable`). This single-inheritance rule is why interfaces matter so much in C#: they're how a type takes on multiple contracts without the diamond-problem ambiguity multiple inheritance creates."
        }
      ],
      challenges: [
        {
      ...
      questions: [
        {
          "id": "oop-q18",
          "kind": "mcq",
          "prompt": "What is the key difference between an interface and an abstract class in C#?",
          "options": [
            {
              "label": "A class can implement MANY interfaces but inherit only ONE base class, and interfaces are pure contracts that (traditionally) carry no instance state",
              "correct": true
            },
            {
              "label": "An interface can have constructors and instance fields, but an abstract class cannot",
              "correct": false
            },
            {
              "label": "Only abstract classes can declare methods; interfaces can only declare properties",
              "correct": false
            },
            {
              "label": "There is no difference — they compile to the same thing",
              "correct": false
            }
          ],
          "explanation": "An interface is a pure contract: it lists members a type promises to provide, carries no instance fields or constructors, and a single type can implement many of them. An abstract class can hold shared state (fields), constructors, and concrete implementations, but a type may inherit only ONE. Reach for an interface when you need a DI/testing seam or multiple unrelated capabilities; reach for an abstract class when related types share state and logic. (Default interface methods, C# 8+, blur the line slightly by letting interfaces ship a default body.)"
        },
        {
          "id": "oop-q19",
          "kind": "predict",
          "prompt": "A default interface method (C# 8+) and how it is reached. What does this print?",
          "code": "IGreeter g = new English();\nConsole.WriteLine(g.Greet());\n\ninterface IGreeter\n{\n    string Greet() => \"Hello\";   // default implementation\n}\n\nclass English : IGreeter\n{\n    // does NOT re-declare Greet()\n}",
          "options": [
            {
              "label": "Hello",
              "correct": true
            ...
            {
              "label": "Compile error — `English` must implement `Greet`",
              "correct": false
            },
            {
              "label": "An empty line",
            ...
            {
              "label": "Compile error — interfaces cannot have method bodies",
              "correct": false
            }
          ],
          "explanation": "Since C# 8, an interface member may carry a DEFAULT body. `English` doesn't re-declare `Greet()`, so it inherits the interface's default and prints `Hello`. Gotcha: a default method that isn't re-implemented by the class is reachable only through an INTERFACE-typed reference — `((English)g).Greet()` would NOT compile because the method isn't part of the class's own surface. Default methods exist mainly to let library authors add to an interface without breaking existing implementers."
        }
      ],
      challenges: [
        {
    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          "id": "oop-q20",
          "kind": "mcq",
          "prompt": "Your teammate says 'records are immutable, so I don't need to worry about anyone changing this data.' Why is that statement only partly true?",
          "options": [
            {
              "label": "Immutability comes from `init`-only/positional members, not from the word `record` — a record with `{ get; set; }` properties is fully mutable",
              "correct": true
            },
            {
              "label": "Records are always mutable; the `with` expression proves it by changing values in place",
              "correct": false
            },
            {
              "label": "Records are immutable only when declared as `record struct`",
              "correct": false
            },
            {
              "label": "Records cannot have properties at all, so there is nothing to mutate",
              "correct": false
            }
          ],
          "explanation": "`record` gives you value equality, a `ToString`, deconstruction, and `with`-expression support — but NOT immutability by itself. Positional parameters become `init`-only (settable only at construction), which IS immutable, but if you write `record class Mutable { public int X { get; set; } }`, the `set` makes it freely mutable. The `with` expression is non-destructive — it builds a NEW record rather than editing the original — so it doesn't contradict immutability. Bottom line: immutability is a property of the members (`init` vs `set`), not of the `record` keyword."
        }
      ],
      challenges: [
        {
      ...
      questions: [
        {
          "id": "oop-q21",
          "kind": "predict",
          "prompt": "Value-type copy semantics. What does this print?",
          "code": "var a = new Tally();\na.Value = 5;\nvar b = a;          // structs copy by VALUE\nb.Value = 99;\nConsole.WriteLine($\"{a.Value} {b.Value}\");\n\nstruct Tally\n{\n    public int Value;\n}",
          "options": [
            {
              "label": "5 99",
              "correct": true
          ...
          "explanation": "`Tally` is a `struct` (value type), so `b = a` copies the WHOLE value, not a reference. `a` and `b` are independent, so mutating `b` leaves `a` at `5` — output `5 99`. Had `Tally` been a `class`, both variables would point to the same heap object and you'd see `99 99`. This copy-by-value behavior is the single most important difference between structs and classes, and the source of the classic 'why didn't my change stick?' bug."
        },
        {
          "id": "oop-q22",
          "kind": "mcq",
          "prompt": "Which is the best rule of thumb for choosing a `struct` over a `class`?",
          "options": [
            {
              "label": "Use a struct for a small, immutable, value-semantic type (roughly <= 16 bytes); use a class for most things, especially anything with identity or mutable shared state",
              "correct": true
            },
            {
              "label": "Always use a struct — they're faster because they never allocate",
              "correct": false
            },
            {
              "label": "Use a struct whenever you need inheritance, since classes can't inherit",
              "correct": false
            },
            {
              "label": "Use a struct for anything that needs to be stored in a `List<T>`",
              "correct": false
            }
          ],
          "explanation": "Structs shine for small, immutable values with value semantics — coordinates, money, a date range — typically up to about 16 bytes. They copy by value (which gets expensive for large structs) and can't participate in class inheritance. Default to a `class` for entities with identity, mutable state, or anything you pass around expecting shared updates. Note: structs CAN be boxed (assigning to `object` heap-allocates a copy), so 'never allocates' is false, and they can implement interfaces but can't inherit from another class."
        }
      ],
      challenges: [
        {
