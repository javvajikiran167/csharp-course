  ...
  lessons: [
    {
      ...
      questions: [
        ...
        },
        {
          "id": "methods-q15",
          "kind": "predict",
          "prompt": "Extracting a method is how you stay DRY. Predict the output of calling the same reusable method twice:",
          "code": "static int Area(int w, int h) => w * h;\n\nConsole.WriteLine($\"{Area(3, 4)} {Area(5, 2)}\");",
          "options": [
            {
              "label": "12 10",
              "correct": true
          ...
          "explanation": "`Area(3, 4)` returns 3×4 = 12 and `Area(5, 2)` returns 5×2 = 10, so the output is `12 10`. The expression-bodied form `=> w * h` is shorthand for `{ return w * h; }`. Defining the multiply logic once and calling it with different arguments is the DRY payoff — the same idea as a Python `def area(w, h): return w * h`, but with explicit `int` parameter and return types checked at compile time."
        }
      ],
      challenges: [
        {
    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          "id": "methods-q16",
          "kind": "mcq",
          "prompt": "Which pair of methods is a **valid overload set** (compiles) in C#?",
          "options": [
            {
              "label": "`int Add(int a, int b)` and `double Add(double a, double b)` — they differ by parameter types.",
              "correct": true
            },
            {
              "label": "`int Add(int a, int b)` and `double Add(int a, int b)` — they differ only by return type.",
              "correct": false
            },
            {
              "label": "`int Add(int a, int b)` and `int Add(int x, int y)` — they differ only by parameter names.",
              "correct": false
            },
            {
              "label": "`int Add(int a, int b)` and `int Add(ref int a, out int b)` differ only by `ref` vs `out`, which is a distinct signature.",
              "correct": false
            }
          ],
          "explanation": "Overloads must differ in the **parameter list** — types, count, or order. Only the first pair does (`int,int` vs `double,double`). Differing solely by **return type** is illegal because the return type is not part of the signature. Differing solely by **parameter names** is illegal because names aren't part of the signature either. And two methods cannot differ **only** by `ref` vs `out` — the compiler treats that as the same signature and reports a duplicate."
        }
      ],
      challenges: [
        {
    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          "explanation": "A **value tuple** return type lets a single expression-bodied method hand back several related values without `out` parameters: `(int min, int max) MinMax(int[] xs) => (xs.Min(), xs.Max());`. Callers can destructure it: `var (lo, hi) = MinMax(data);`. The element names (`min`, `max`) are optional but make the result self-documenting — much nicer than returning multiple `out` params."
        }
      ],
      challenges: [
        {
